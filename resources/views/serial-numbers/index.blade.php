<x-app-layout>
    <div class="mb-4 d-flex justify-content-between align-items-center">
        <div>
            <h1 class="h2 fw-bold mb-1" style="color: #1f2937;">Serial Numbers</h1>
            <p class="text-muted mb-0">View and manage serial numbers and khata numbers for all Proforma Invoices</p>
        </div>
        <div>
    ...
    <div class="card shadow-sm border-0 mb-4" style="background: linear-gradient(to bottom, #ffffff 0%, color-mix(in srgb, var(--primary-color) 6%, #ffffff) 100%); border-radius: 12px;">
        <div class="card-header border-0 pb-0" style="background: transparent;">
            <div class="d-flex align-items-center py-3 border-bottom" style="border-color: color-mix(in srgb, var(--primary-color) 20%, transparent) !important;">
                ...
                <h2 class="h5 fw-bold mb-0" style="color: #1f2937;">Search Serial Numbers</h2>
            </div>
        </div>
        <div class="card-body p-4">
            <form method="GET" action="{{ route('serial-numbers.index') }}" id="searchForm">
                <div class="row g-3">
                    <div class="col-md-4">
        ...
        <div class="card-body p-4">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex gap-2">
                                        ...
                                        <a href="{{ route('serial-numbers.show', $pi) }}" class="btn btn-sm {{ $pi->serialNumbers && $pi->serialNumbers->count() > 0 ? 'btn-info' : 'btn-success' }}" title="{{ $pi->serialNumbers && $pi->serialNumbers->count() > 0 ? 'View/Edit' : 'Add' }} Serial Numbers">
            ...
            @else
                <div class="text-center py-5">
                    ...
                    <p class="text-muted small">Try adjusting your filters or <a href="{{ route('proforma-invoices.index') }}">view all PIs</a> to add serial numbers.</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            salesManagerSelect.addEventListener('change', function() {
                ...
                if (salesManagerId) {
                    // Fetch PIs
                    fetch(`{{ route('serial-numbers.get-pis') }}?sales_manager_id=${salesManagerId}`)
                    ...
                    // Fetch Customers
                    fetch(`{{ route('serial-numbers.get-customers') }}?sales_manager_id=${salesManagerId}`)
                        .then(response => response.json())
                        .then(data => {
