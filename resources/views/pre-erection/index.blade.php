<x-app-layout>
    <div class="mb-4 d-flex justify-content-between align-items-center">
        <div>
            ...
            <p class="text-muted mb-0">View and manage pre-erection details for all Proforma Invoices</p>
        </div>
        <div>
    ...
    <div class="card shadow-sm border-0 mb-4" style="background: linear-gradient(to bottom, #ffffff 0%, color-mix(in srgb, var(--primary-color) 6%, #ffffff) 100%); border-radius: 12px;">
        <div class="card-header border-0 pb-0" style="background: transparent;">
            <div class="d-flex align-items-center py-3 border-bottom" style="border-color: color-mix(in srgb, var(--primary-color) 20%, transparent) !important;">
                ...
                <h2 class="h5 fw-bold mb-0" style="color: #1f2937;">Search Pre Erection Details</h2>
            </div>
        </div>
        <div class="card-body p-4">
            <form method="GET" action="{{ route('pre-erection.index') }}" id="searchForm">
                <div class="row g-3">
                    <div class="col-md-4">
        ...
        <div class="card-body p-4">
                    ...
                    <table class="table table-bordered table-hover">
                        <thead style="background: linear-gradient(45deg, var(--primary-color), var(--primary-light)); color: white;">
                            <tr>
                                ...
                                <th>Sales Manager</th>
                                <th>Total Amount</th>
                                <th>Pre Erection Details Count</th>
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @php
                                        $completedCount = $pi->preErectionDetails ? $pi->preErectionDetails->where('is_completed', true)->count() : 0;
                                        $totalCount = $pi->preErectionDetails ? $pi->preErectionDetails->count() : 0;
                                    @endphp
                                    @if($completedCount > 0)
                                        <span class="badge bg-success">{{ $completedCount }} Completed</span>
                                        @if($totalCount > $completedCount)
                                            <span class="badge bg-secondary ms-1">{{ $totalCount - $completedCount }} Pending</span>
                                        @endif
                                    @elseif($totalCount > 0)
                                ...
                                <td>
                                    <div class="d-flex gap-2">
                                        ...
                                        <a href="{{ route('pre-erection.show', $pi) }}" class="btn btn-sm {{ $pi->preErectionDetails && $pi->preErectionDetails->count() > 0 ? 'btn-info' : 'btn-success' }}" title="{{ $pi->preErectionDetails && $pi->preErectionDetails->count() > 0 ? 'Edit' : 'Add' }} Pre Erection Details">
            ...
            @else
                <div class="text-center py-5">
                    ...
                    <p class="text-muted small">Try adjusting your filters or <a href="{{ route('proforma-invoices.index') }}">view all PIs</a> to add pre-erection details.</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            salesManagerSelect.addEventListener('change', function() {
                ...
                if (salesManagerId) {
                    // Fetch PIs
                    fetch(`{{ route('pre-erection.get-pis') }}?sales_manager_id=${salesManagerId}`)
                    ...
                    // Fetch Customers
                    fetch(`{{ route('pre-erection.get-customers') }}?sales_manager_id=${salesManagerId}`)
                        .then(response => response.json())
                        .then(data => {
