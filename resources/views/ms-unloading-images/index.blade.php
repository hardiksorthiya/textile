<x-app-layout>
    <div class="mb-4 d-flex justify-content-between align-items-center">
        <div>
            ...
            <p class="text-muted mb-0">View and manage uploaded images for all Proforma Invoices</p>
        </div>
        <div>
    ...
    <div class="card shadow-sm border-0 mb-4" style="background: linear-gradient(to bottom, #ffffff 0%, color-mix(in srgb, var(--primary-color) 6%, #ffffff) 100%); border-radius: 12px;">
        <div class="card-header border-0 pb-0" style="background: transparent;">
            <div class="d-flex align-items-center py-3 border-bottom" style="border-color: color-mix(in srgb, var(--primary-color) 20%, transparent) !important;">
                ...
                <h2 class="h5 fw-bold mb-0" style="color: #1f2937;">Search Images</h2>
            </div>
        </div>
        <div class="card-body p-4">
            <form method="GET" action="{{ route('ms-unloading-images.index') }}" id="searchForm">
                <div class="row g-3">
                    <div class="col-md-4">
        ...
        <div class="card-body p-4">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex gap-2">
                                        ...
                                        <a href="{{ route('ms-unloading-images.show', $pi) }}" class="btn btn-sm {{ $pi->msUnloadingImages && $pi->msUnloadingImages->count() > 0 ? 'btn-info' : 'btn-success' }}" title="{{ $pi->msUnloadingImages && $pi->msUnloadingImages->count() > 0 ? 'View/Upload' : 'Upload' }} Images">
            ...
            @else
                <div class="text-center py-5">
                    ...
                    <p class="text-muted small">Try adjusting your filters or <a href="{{ route('proforma-invoices.index') }}">view all PIs</a> to upload images.</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            salesManagerSelect.addEventListener('change', function() {
                ...
                if (salesManagerId) {
                    // Fetch PIs
                    fetch(`{{ route('ms-unloading-images.get-pis') }}?sales_manager_id=${salesManagerId}`)
                    ...
                    // Fetch Customers
                    fetch(`{{ route('ms-unloading-images.get-customers') }}?sales_manager_id=${salesManagerId}`)
                        .then(response => response.json())
                        .then(data => {
